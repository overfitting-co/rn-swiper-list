...
import Animated, { cancelAnimation, interpolate, runOnJS, useAnimatedStyle, useDerivedValue, useSharedValue, withSpring, withTiming } from 'react-native-reanimated';
import OverlayLabel from './OverlayLabel';
const SwipeableCard = /*#__PURE__*/forwardRef(({
  index,
  activeIndex,
  onSwipeLeft,
  onSwipeRight,
  ...
  const rotateX = useDerivedValue(() => {
    return interpolate(translateX.value, rotateInputRange ?? [], rotateOutputRange ?? [], 'clamp');
  }, [inputRangeX]);
  const gesture = Gesture.Pan().onBegin(() => {
    ...
    if (height / 3 < Math.abs(event.translationY)) {
      nextActiveIndex.value = interpolate(translateY.value, inputRangeY, [currentActiveIndex.value + 1, currentActiveIndex.value, currentActiveIndex.value + 1], 'clamp');
      return;
    }
    nextActiveIndex.value = interpolate(translateX.value, inputRangeX, [currentActiveIndex.value + 1, currentActiveIndex.value, currentActiveIndex.value + 1], 'clamp');
  }).onFinalize(event => {
    if (currentActiveIndex.value !== index) return;
    if (onSwipeEnd) runOnJS(onSwipeEnd)();
    if (nextActiveIndex.value === activeIndex.value + 1) {
      const sign = Math.sign(event.translationX);
      const signPositionY = Number.isInteger(interpolate(translateY.value, inputRangeY, [currentActiveIndex.value + 1, currentActiveIndex.value, currentActiveIndex.value + 1], 'clamp'));
  ...
  }, OverlayLabelLeft && /*#__PURE__*/React.createElement(OverlayLabel, {
...
export default /*#__PURE__*/memo(SwipeableCard);
